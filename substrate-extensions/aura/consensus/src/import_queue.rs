...
use crate::{AuthoritiesTracker, AuthorityId, LOG_TARGET, authorities};
	...
	ID: InherentDigest + Send + Sync + 'static,
{
	async fn verify(
		...
		let authorities_from_runtime = authorities(
			self.client.as_ref(),
			parent_hash,
			*block.header.number(),
		...
		let authorities_from_tracker = self
			.authorities_tracker
		...
		info!("Authorities from runtime:");
		for a in authorities_from_runtime.iter() {
			info!("    {a:?}")
		}
		info!("Authorities from tracker:");
		for a in authorities_from_tracker.iter() {
			info!("    {a:?}")
		}
