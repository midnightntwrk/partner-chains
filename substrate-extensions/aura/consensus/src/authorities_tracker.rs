...
use crate::{AuthorityId, CompatibilityMode, fetch_authorities_from_runtime};
use fork_tree::ForkTree;
use parking_lot::RwLock;
use sp_api::ProvideRuntimeApi;
use sp_blockchain::{HeaderBackend, HeaderMetadata};
use sp_consensus_aura::{AURA_ENGINE_ID, AuraApi, ConsensusLog};
use sp_core::Pair;
use sp_runtime::{
	codec::Codec,
		...
		match authorities {
			...
			None => {
				...
				authorities_cache
					...
					.map_err(|e| {
						format!(
							"Could not import authorities for block {parent_hash:?} at number {}: {e}",
							number - 1u32.into()
						)
					})?;
				Ok(authorities)
	...
	P::Public: Codec,
{
	for log in header.digest().logs() {
		...
		if let Some(ConsensusLog::AuthoritiesChange(authorities)) = log {
			log::info!("found {} authorities in digest", authorities.len());
			return Some(authorities);
		}
	}
	log::info!("DID NOT found authorities in digest");
	None
}
