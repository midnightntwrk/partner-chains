...
#[tokio::test]
async fn can_upsert_versioned_script_twice() {
	let image = GenericImage::new(TEST_IMAGE, TEST_IMAGE_TAG);
	let container = image.start().await.unwrap();
	let client = initialize(&container).await;
	let genesis_utxo = run_init_governance(&client).await;
	...
	assert_eq!(versioned_utxo.script.unwrap().cbor, plutus_script_1.bytes);
...
#[tokio::test]
async fn test_upsert_versioned_script_invalid_script_id() {
	let image = GenericImage::new(TEST_IMAGE, TEST_IMAGE_TAG);
	let container = image.start().await.unwrap();
	let client = initialize(&container).await;
	let genesis_utxo = run_init_governance(&client).await;
	let plutus_script = plutus_script![raw_scripts::ALWAYS_FAILING_POLICY, 0u64].unwrap();
	let result = run_upsert_versioned_script(
		plutus_script.clone(),
		999999u32,
		genesis_utxo,
		&eve_payment_key(),
	...
	.await;
	assert!(result.is_err(), "Upserting a script with invalid script id should fail");
}
