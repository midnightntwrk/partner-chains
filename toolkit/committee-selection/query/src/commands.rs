...
mod tests {
	...
	use sidechain_domain::{
		McBlockNumber, McSlotNumber, McTxHash, McTxIndexInBlock, SidechainPublicKey,
		StakePoolPublicKey, UtxoId, UtxoIndex, UtxoInfo, byte_string::ByteString,
	};
	...
	async fn cli_get_registration_status_returns_correct_json_string() {
		...
		let response = vec![CandidateRegistrationEntry {
			...
			keys: vec![("safr".to_string(), aura),("gran".to_string(), grandpa)].into_iter().collect(),
